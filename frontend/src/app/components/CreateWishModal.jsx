...
const CreateWishModal = () => {
  ...
  const handleCreateWish = async () => {
    ...
    try {
      const response = await axios.post(
        `${config.BASE_API_URL_DOCKER}/wishes/`,
        {
          name,
        },
        axios_headers
  ...
  return (
    <>
      <Link
        onPress={onOpen}
        className="create-wish-link mt-16 text-xl cursor-pointer"
      >
      ...
      >
        <ModalContent>
          {(onOpen) => (
            <>
              <ModalHeader className="flex flex-col text-2xl justify-center items-center">
                Craft Your Own New Year Wish 🎉
              </ModalHeader>
